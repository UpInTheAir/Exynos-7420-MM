...
#define NOBLE_AUD_PLL_FREQ	393216018

#define NOBLE_44K_MCLK1	22579200
#define NOBLE_48K_MCLK1	24000000
#define NOBLE_DEFAULT_MCLK2	32768

#define NOBLE_RUN_MAINMIC	2
#define NOBLE_RUN_EARMIC	1

/* XXX_FLLn is CODEC_FLLn */
#define NOBLE_FLL1        1
#define NOBLE_FLL2        2
#define NOBLE_FLL1_REFCLK 3
#define NOBLE_FLL2_REFCLK 4
#define NOBLE_FLL3        5
#define NOBLE_FLL3_REFCLK 6
...
#define NOBLE_AIF_MAX	4
#else
#define NOBLE_AIF_MAX	3
#endif
...
struct arizona_machine_priv {
	...
	struct snd_soc_dai *aif[NOBLE_AIF_MAX];
	int voice_uevent;
	...
	int arizona_clk_src_mclk;
	int arizona_clk_asrc_refmclk;
	int arizona_clk_asrc_mclk;
	int fll2_base_mclk;
	int base_mclk;
	int sysclk_rate;
	int dspclk_rate;
...
static struct arizona_machine_priv noble_wm5110_priv = {
	.arizona_clk_src_mclk = ARIZONA_CLK_SRC_MCLK1,
	.arizona_clk_asrc_refmclk = ARIZONA_FLL_SRC_NONE,
	.arizona_clk_asrc_mclk = ARIZONA_CLK_SRC_MCLK2,
	.fll2_base_mclk = NOBLE_DEFAULT_MCLK2,
	.base_mclk = NOBLE_48K_MCLK1,
...
static struct arizona_machine_priv noble_wm1840_priv = {
	.arizona_clk_src_mclk = ARIZONA_CLK_SRC_MCLK1,
	.arizona_clk_asrc_refmclk = ARIZONA_FLL_SRC_NONE,
	.arizona_clk_asrc_mclk = ARIZONA_CLK_SRC_MCLK2,
	.fll2_base_mclk = NOBLE_DEFAULT_MCLK2,
	.base_mclk = NOBLE_DEFAULT_MCLK2,
...
static const struct snd_soc_component_driver noble_cmpnt = {
	.name	= "noble-audio",
};
...
static void mclk_control(struct snd_soc_card *card, enum mclk_src mclk_source)
{
	struct arizona_machine_priv *priv = card->drvdata;
	static unsigned int mclk_onoff = 0;
	int ret;

	dev_info(card->dev, " %s mclk_source=%d, onoff = %d\n",
		__func__, mclk_source, mclk_onoff);

	if (mclk_source & CLK_24M) {
		if (priv->mclk && !mclk_onoff) {
	...
	if (gpio_is_valid(priv->clk_sel_gpio))
		gpio_set_value(priv->clk_sel_gpio, mclk_source & CLK_22M);
}

static void noble_change_mclk(struct snd_soc_card *card, int rate)
{
	struct arizona_machine_priv *priv = card->drvdata;
	int change_base_mclk;
	int ret;

	dev_info(card->dev, " %s rate=%d\n", __func__, rate);

	if (rate == 0)
		change_base_mclk = NOBLE_DEFAULT_MCLK2;
	else if (rate % 8000)
		change_base_mclk = NOBLE_44K_MCLK1;
	else
		change_base_mclk = NOBLE_48K_MCLK1;

	if (priv->base_mclk == change_base_mclk) {
		dev_info(card->dev, "skip mclk control freq %d\n", priv->base_mclk);
		return;
	}

	switch (change_base_mclk) {
	case NOBLE_48K_MCLK1:
		/* enable 24M, disable 22.579M and switch 24M port */
		mclk_control(card, CLK_24M);
		priv->sysclk_rate = 98304000;
		priv->arizona_clk_src_mclk = ARIZONA_CLK_SRC_MCLK1;
		dev_info(card->dev, "change mclk as 24M\n");
		break;
	case NOBLE_44K_MCLK1:
		/* disable 24M, enable 22.579M and switch 22.579M port */
		mclk_control(card, CLK_22M);
		priv->sysclk_rate = 90316800;
		priv->arizona_clk_src_mclk = ARIZONA_CLK_SRC_MCLK1;
		dev_info(card->dev, "change mclk as 22.579M\n");
		break;
	case NOBLE_DEFAULT_MCLK2:
		/* disable 24M, disable 22.579M and switch 22.579M port */
		mclk_control(card, CLK_32K);
		priv->sysclk_rate = 98304000;
		priv->arizona_clk_src_mclk = ARIZONA_CLK_SRC_MCLK2;
		dev_info(card->dev, "change mclk as 32K\n");
		ret = snd_soc_codec_set_pll(priv->codec, NOBLE_FLL3,
				    priv->arizona_clk_src_mclk,
				    change_base_mclk,
				    priv->sysclk_rate);
		if (ret != 0)
			dev_err(card->dev, "Failed to start FLL3: %d\n", ret);
		ret = snd_soc_codec_set_sysclk(priv->codec, ARIZONA_CLK_SYSCLK,
						   ARIZONA_CLK_SRC_FLL3,
						   priv->sysclk_rate,
						   SND_SOC_CLOCK_IN);
		if (ret < 0)
			dev_err(card->dev,
		...
		break;
	default:
		dev_info(card->dev, "fail to change_base_mclk\n");
	}

	priv->base_mclk = change_base_mclk;

	return;
}

void noble_arizona_hpdet_cb(unsigned int meas)
{
...
void noble_arizona_micd_cb(bool mic)
{
	struct arizona_machine_priv *priv;
...
void noble_update_impedance_table(struct device_node *np)
{
...
static int noble_dsm_cal_apply(struct snd_soc_card *card)
{
	struct arizona_machine_priv *priv = card->drvdata;
...
static int noble_external_amp(struct snd_soc_dapm_widget *w,
...
static int noble_ext_mainmicbias(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_card *card = w->dapm->card;
	struct arizona_machine_priv *priv = card->drvdata;
...
static const struct snd_kcontrol_new noble_codec_controls[] = {
...
static const struct snd_kcontrol_new noble_controls[] = {
...
const struct snd_soc_dapm_widget noble_dapm_widgets[] = {
	SND_SOC_DAPM_HP("HP", NULL),
	SND_SOC_DAPM_SPK("SPK", noble_external_amp),
	SND_SOC_DAPM_SPK("RCV", NULL),

	SND_SOC_DAPM_MIC("Headset Mic", NULL),
	SND_SOC_DAPM_MIC("Main Mic", noble_ext_mainmicbias),
...
const struct snd_soc_dapm_route noble_wm1840_dapm_routes[] = {
	{ "HP", NULL, "HPOUT1L" },
	{ "HP", NULL, "HPOUT1R" },
...
static int noble_aif_startup(struct snd_pcm_substream *substream)
{
...
static void noble_aif_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
	...
	dev_info(card->dev, "%s-%d shutdown++ codec_dai[%s]:playback=%d codec_active=%d\n",
			rtd->dai_link->name, substream->stream,
			codec_dai->name, codec_dai->playback_active,
			codec->active);

	if (!codec_dai->playback_active && !codec_dai->capture_active && !codec->active)
		noble_change_mclk(card, 0);

	return;
}

static int noble_aif1_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params)
{
	...
	noble_change_mclk(card, params_rate(params));

	/* Set FLL3 */
	ret = snd_soc_codec_set_pll(priv->codec, NOBLE_FLL3_REFCLK,
				    ARIZONA_FLL_SRC_NONE, 0, 0);
	if (ret != 0)
		dev_err(card->dev, "Failed to start FLL3 REF: %d\n", ret);
	...
	/* Set FLL2 */
	if(priv->arizona_clk_asrc_refmclk == ARIZONA_FLL_SRC_NONE) {
		ret = snd_soc_codec_set_pll(priv->codec, NOBLE_FLL2_REFCLK,
					    priv->arizona_clk_asrc_refmclk, 0 ,0);
	} else {
		ret = snd_soc_codec_set_pll(priv->codec, NOBLE_FLL2_REFCLK,
					    priv->arizona_clk_asrc_refmclk,
					    priv->base_mclk,
					    priv->asyncclk_rate);
	}
	...
	ret = snd_soc_codec_set_pll(priv->codec, NOBLE_FLL2,
				    priv->arizona_clk_asrc_mclk,
				    priv->fll2_base_mclk,
				    priv->asyncclk_rate);
	if (ret != 0)
		dev_err(card->dev, "Failed to start FLL2: %d\n", ret);

	/* Set ASYNCCLK from FLL2 */
	ret = snd_soc_codec_set_sysclk(priv->codec, ARIZONA_CLK_ASYNCCLK,
...
static int noble_aif1_hw_free(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
...
static int noble_aif1_prepare(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
...
static int noble_aif1_trigger(struct snd_pcm_substream *substream, int cmd)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
...
static struct snd_soc_ops noble_aif1_ops = {
	.startup = noble_aif_startup,
	.shutdown = noble_aif_shutdown,
	.hw_params = noble_aif1_hw_params,
	.hw_free = noble_aif1_hw_free,
	.prepare = noble_aif1_prepare,
	.trigger = noble_aif1_trigger,
};

#ifdef CONFIG_SND_SAMSUNG_COMPR
static void noble_compress_shutdown(struct snd_compr_stream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
	struct snd_soc_card *card = rtd->card;
	...
	dev_info(card->dev, "%s shutdown++ codec_dai[%s]:playback=%d codec_active=%d\n",
			...
			codec_dai->playback_active, codec->active);
...
static struct snd_soc_compr_ops noble_compress_ops = {
	.shutdown = noble_compress_shutdown,
};
#endif

static int noble_aif2_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
	...
	if (priv->aif_format[1] & SND_SOC_DAIFMT_CBS_CFS) {
		priv->arizona_clk_asrc_refmclk = ARIZONA_FLL_SRC_MCLK1;
		priv->arizona_clk_asrc_mclk = ARIZONA_FLL_SRC_AIF2BCLK;
		priv->fll2_base_mclk = bclk;
	} else {
		priv->arizona_clk_asrc_refmclk = ARIZONA_FLL_SRC_NONE;
		priv->arizona_clk_asrc_mclk = ARIZONA_CLK_SRC_MCLK1;
		priv->fll2_base_mclk = priv->base_mclk;
	}
...
static int noble_aif2_hw_free(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
	struct arizona_machine_priv *priv = rtd->card->drvdata;
	...
	priv->arizona_clk_asrc_refmclk = ARIZONA_FLL_SRC_NONE;
	priv->arizona_clk_asrc_mclk = ARIZONA_CLK_SRC_MCLK2;
	priv->fll2_base_mclk = NOBLE_DEFAULT_MCLK2;

	return 0;
}

static struct snd_soc_ops noble_aif2_ops = {
	.shutdown = noble_aif_shutdown,
	.hw_params = noble_aif2_hw_params,
	.hw_free = noble_aif2_hw_free,
};

static int noble_aif3_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
...
static struct snd_soc_ops noble_aif3_ops = {
	.shutdown = noble_aif_shutdown,
	.hw_params = noble_aif3_hw_params,
};

static struct snd_soc_dai_driver noble_ext_dai[] = {
	{
		.name = "noble-ext voice call",
		.playback = {
			.channels_min = 1,
			.channels_max = 4,
			.rate_min = 8000,
...
static struct snd_soc_dai_link noble_wm5110_dai[] = {
	{ /* playback & recording */
		.name = "playback-pri",
	...
	{ /* voice call */
		.name = "baseband",
		.stream_name = "noble-ext voice call",
		.cpu_dai_name = "noble-ext voice call",
	...
	{ /* bluetooth sco */
		.name = "bluetooth sco",
		.stream_name = "noble-ext bluetooth sco",
		.cpu_dai_name = "noble-ext bluetooth sco",
		.platform_name = "snd-soc-dummy",
...
static struct snd_soc_dai_link noble_wm1840_dai[] = {
	{ /* playback & recording */
		.name = "playback-pri",
		.stream_name = "i2s0-pri",
...
static int noble_of_get_pdata(struct snd_soc_card *card)
{
	...
	noble_update_impedance_table(pdata_np);
...
static int noble_late_probe(struct snd_soc_card *card)
{
	...
	noble_of_get_pdata(card);

	for (i = 0; i < NOBLE_AIF_MAX; i++)
	...
	ret = snd_soc_add_codec_controls(codec, noble_codec_controls,
					ARRAY_SIZE(noble_codec_controls));
	if (ret < 0) {
		dev_err(codec->dev,
	...
	/* Set FLL3 */
	ret = snd_soc_codec_set_pll(codec, NOBLE_FLL3_REFCLK,
				    ARIZONA_FLL_SRC_NONE, 0, 0);
	if (ret != 0)
		dev_err(card->dev,
				"Failed to start FLL3 REF: %d\n", ret);

	ret = snd_soc_codec_set_pll(codec, NOBLE_FLL3,
	...
	arizona_set_hpdet_cb(codec, noble_arizona_hpdet_cb);
	arizona_set_micd_cb(codec, noble_arizona_micd_cb);
...
static int noble_start_sysclk(struct snd_soc_card *card)
{
	struct arizona_machine_priv *priv = card->drvdata;
	struct snd_soc_codec *codec = priv->codec;
	int ret;
	...
	if (priv->dspclk_rate > 0) {
		/* Set FLL1 */
		ret = snd_soc_codec_set_pll(codec, NOBLE_FLL1_REFCLK,
					    ARIZONA_FLL_SRC_NONE, 0, 0);
		if (ret != 0)
			dev_err(card->dev,
					"Failed to start FLL1 REF: %d\n", ret);

		ret = snd_soc_codec_set_pll(codec, NOBLE_FLL1,
					    ARIZONA_CLK_SRC_MCLK2,
					    NOBLE_DEFAULT_MCLK2,
					    priv->asyncclk_rate);
		if (ret != 0) {
...
static int noble_stop_sysclk(struct snd_soc_card *card)
{
	struct arizona_machine_priv *priv = card->drvdata;
	int ret;
	struct snd_soc_dai *codec_dai = card->rtd[0].codec_dai;
	...
	/* Power down FLL3 */
	ret = snd_soc_codec_set_pll(priv->codec, NOBLE_FLL3, 0, 0, 0);
	if (ret != 0)
		dev_err(card->dev, "Failed to stop FLL3: %d\n", ret);

	/* Power down FLL2 */
	ret = snd_soc_codec_set_pll(priv->codec, NOBLE_FLL2, 0, 0, 0);
	if (ret != 0)
		dev_err(card->dev, "Failed to stop FLL2: %d\n", ret);

	/* Power down FLL1 */
	ret = snd_soc_codec_set_pll(priv->codec, NOBLE_FLL1, 0, 0, 0);
	if (ret != 0)
...
static int noble_set_bias_level(struct snd_soc_card *card,
				...
				enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		if (card->dapm.bias_level == SND_SOC_BIAS_OFF)
			noble_start_sysclk(card);
		break;
	case SND_SOC_BIAS_OFF:
		noble_stop_sysclk(card);
		break;
...
static int noble_set_bias_level_post(struct snd_soc_card *card,
				     struct snd_soc_dapm_context *dapm,
				     enum snd_soc_bias_level level)
{
	dev_dbg(card->dev, "%s: %d\n", __func__, level);
...
static int noble_change_sysclk(struct snd_soc_card *card, int source)
{
	struct arizona_machine_priv *priv = card->drvdata;
	struct snd_soc_codec *codec = priv->codec;
	int ret;
...
static int noble_check_clock_conditions(struct snd_soc_card *card)
{
	struct snd_soc_codec *codec = card->rtd[0].codec;
	struct arizona_machine_priv *priv = card->drvdata;
	int mainmic_state = 0;
	...
	if (!codec->active && mainmic_state) {
		...
		return NOBLE_RUN_MAINMIC;
	}

	if (!codec->active && priv->ear_mic && !mainmic_state) {
		dev_info(card->dev, "EAR_MIC is running without input stream\n");
		return NOBLE_RUN_EARMIC;
	}

	return 0;
}

static int noble_suspend_post(struct snd_soc_card *card)
{
	struct arizona_machine_priv *priv = card->drvdata;
	int ret;
	...
	ret = noble_check_clock_conditions(card);

	if (ret == NOBLE_RUN_EARMIC) {
		arizona_enable_force_bypass(priv->codec);
		noble_stop_sysclk(card);
		dev_info(card->dev, "%s: stop_sysclk\n", __func__);
	} else if (ret == NOBLE_RUN_MAINMIC) {
		arizona_enable_force_bypass(priv->codec);
		noble_change_sysclk(card, 0);
...
static int noble_resume_pre(struct snd_soc_card *card)
{
	struct arizona_machine_priv *priv = card->drvdata;
	int ret;
...
static struct snd_soc_card noble_cards[] = {
	{
		.name = "Noble WM5110 Sound",
		.owner = THIS_MODULE,

		.dai_link = noble_wm5110_dai,
		.num_links = ARRAY_SIZE(noble_wm5110_dai),

		.controls = noble_controls,
		.num_controls = ARRAY_SIZE(noble_controls),
		.dapm_widgets = noble_dapm_widgets,
		.num_dapm_widgets = ARRAY_SIZE(noble_dapm_widgets),

		.late_probe = noble_late_probe,

		.suspend_post = noble_suspend_post,
		.resume_pre = noble_resume_pre,

		.set_bias_level = noble_set_bias_level,
		.set_bias_level_post = noble_set_bias_level_post,

		.drvdata = &noble_wm5110_priv,
	},
	{
		.name = "Noble WM1840 Sound",
		.owner = THIS_MODULE,

		.dai_link = noble_wm1840_dai,
		.num_links = ARRAY_SIZE(noble_wm1840_dai),

		.controls = noble_controls,
		.num_controls = ARRAY_SIZE(noble_controls),
		.dapm_widgets = noble_dapm_widgets,
		.num_dapm_widgets = ARRAY_SIZE(noble_dapm_widgets),
		.dapm_routes = noble_wm1840_dapm_routes,
		.num_dapm_routes = ARRAY_SIZE(noble_wm1840_dapm_routes),
...
static int noble_audio_probe(struct platform_device *pdev)
{
	int n, ret;
	...
	ret = snd_soc_register_component(card->dev, &noble_cmpnt,
				noble_ext_dai, ARRAY_SIZE(noble_ext_dai));
	if (ret != 0)
...
static int noble_audio_remove(struct platform_device *pdev)
{
...
static const struct of_device_id noble_arizona_of_match[] = {
	{ .compatible = "samsung,noble-arizona", },
	{ },
};
MODULE_DEVICE_TABLE(of, noble_arizona_of_match);

static struct platform_driver noble_audio_driver = {
	.driver	= {
		.name	= "noble-audio",
		.owner	= THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = of_match_ptr(noble_arizona_of_match),
	},
	.probe	= noble_audio_probe,
	.remove	= noble_audio_remove,
};

module_platform_driver(noble_audio_driver);

MODULE_DESCRIPTION("ALSA SoC NOBLE ARIZONA");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:noble-audio");
